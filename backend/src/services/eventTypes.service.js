...
export async function listEventTypes(filters, userId) {
  if (filters?.slug) {
    return eventTypeRepository.getEventTypeBySlug(filters.slug, userId);
  }

  return eventTypeRepository.listEventTypes({ ...filters, userId });
}

export async function createEventType(payload, userId) {
  try {
...
export async function deleteEventType(id, userId) {
  const eventTypes = await eventTypeRepository.listEventTypes({ id, userId });
  ...
  const result = await eventTypeRepository.deleteEventType(id, userId);
