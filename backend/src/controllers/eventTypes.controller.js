...
export async function listEventTypes(req, res, next) {
  try {
    const data = await eventTypeService.listEventTypes(req.query, req.user.id);
    return ok(res, data);
  } catch (error) {
    return next(error);
  }
...
export async function createEventType(req, res, next) {
  try {
    const data = await eventTypeService.createEventType(req.body, req.user.id);
...
export async function updateEventType(req, res, next) {
  try {
    const data = await eventTypeService.updateEventType(req.params.id, req.body, req.user.id);
...
export async function deleteEventType(req, res, next) {
  try {
    const data = await eventTypeService.deleteEventType(req.params.id, req.user.id);
