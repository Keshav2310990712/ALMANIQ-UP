...
export async function listBookings(req, res, next) {
  try {
    const data = await bookingService.listBookings(req.query, req.user.id);
    return ok(res, data);
  } catch (error) {
    return next(error);
  }
...
export async function listAvailableSlots(req, res, next) {
  try {
    const data = await bookingService.listAvailableSlots(req.query, req.user.id);
    return ok(res, data);
  } catch (error) {
    return next(error);
  }
...
export async function cancelBooking(req, res, next) {
  try {
    const data = await bookingService.cancelBooking(req.params.uid, req.user.id);
...
export async function rescheduleBooking(req, res, next) {
  try {
    const data = await bookingService.rescheduleBooking(req.params.uid, req.body, req.user.id);
